...
def test_nutrition_meal_contract_and_plan_gate() -> None:
    ...
    # unauth -> 401 INVALID_ACCESS_TOKEN（実挙動）
    anon = _make_client()
    unauth = _request(
        anon,
        "NUTRITION unauth",
    ...
    # ★ profile無しだと USER_NOT_FOUND で落ちるため先に作る
    ...
    # ★ この期待を通したい場合は、PremiumFeatureRequiredError を 403 にマッピングする必要あり
...
def test_daily_report_contract_happy_and_errors() -> None:
    ...
    gated = _make_client()
    u2 = _register_and_login(gated)
    _put_profile(gated, meals_per_day=1)         # ★ profile無しだと USER_NOT_FOUND
    _create_target(gated, title="Target gated")  # ★ 先に用意しておく（順序依存回避）
