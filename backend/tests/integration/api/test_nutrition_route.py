...
class TestGetMealAndDailyNutrition:
    ...
    def test_get_meal_and_daily_nutrition_success_main(
        self,
        client: TestClient,
        meal_uow: FakeMealUnitOfWork,
        ...
        assert data["meal"]["meal_type"] == "main"
        assert data["meal"]["meal_index"] == 1
        assert len(data["meal"]["nutrients"]) > 0
        assert len(data["daily"]["nutrients"]) > 0

    def test_get_meal_and_daily_nutrition_success_snack(
        self,
        client: TestClient,
        meal_uow: FakeMealUnitOfWork,
        authenticated_user: tuple[User, TokenPair],
        ...
        response = client.get(
            "/api/v1/nutrition/meal?date=2024-01-01&meal_type=snack",
            cookies={
                "ACCESS_TOKEN": tokens.access_token,
            },
        )
        ...
        assert "meal" in data
        assert "daily" in data
        assert data["meal"]["meal_type"] == "snack"
        assert data["meal"]["meal_index"] is None

    def test_get_meal_and_daily_nutrition_empty_entries(
        self,
        client: TestClient,
        authenticated_user: tuple[User, TokenPair],
    ):
        """正常系: 食事ログが0件の場合（栄養素は全て0で返される）"""
        ...
        assert "meal" in data
        assert "daily" in data
        # エントリが0件の場合でも、全ての栄養素が0で返される
        assert len(data["meal"]["nutrients"]) == 10
        # 全ての栄養素の値が0であることを確認
        for nutrient in data["meal"]["nutrients"]:
            assert nutrient["value"] == 0.0
