...
def _create_user(email: str, password: str, password_hasher, clock) -> User:
    return User(
        id=UserId("user-1"),
...
def test_login_success(user_repo, password_hasher, token_service, clock):
...
def test_login_invalid_email(user_repo, password_hasher, token_service, clock):
    # ユーザーは登録されていない
    use_case = LoginUserUseCase(
        user_repo=user_repo,
        password_hasher=password_hasher,
...
def test_login_invalid_password(user_repo, password_hasher, token_service, clock):
